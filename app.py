...
def get_chat_sessions(backend_url):
    response = requests.get(f'{backend_url}/chat_sessions/')
    if response.status_code == 200:
        return response.json()
    else:
        return None
    
def get_chat_messages(backend_url, session_id):
    response = requests.get(f'{backend_url}/chat_messages/{session_id}')
    if response.status_code == 200:
        return response.json()
    else:
        return None
...
def main():
    ...
    if selected == 'Chat':
        ...
        # ファイルの読み込み
        if uploaded_files:
            index = pdf_file_process(uploaded_files)
    ...
    # 'history'ページ
        ...
        # 全てのチャット履歴の取得
        chat_sessions = get_chat_sessions(backend_url)
        ...
        if chat_sessions is not None:
            if not chat_sessions:
                ...
                for session in chat_sessions:
                    with st.expander(f'チャット履歴：{session["chat_name"]}'):
                        # チャット履歴内のすべてのメッセージを取得
                        messages = get_chat_messages(backend_url, session["id"])
